...
class RMGA:
    """
    人民公安报爬虫
    示例网站：https://epaper.cpd.com.cn/szb/wwwcpd_9/dzb_16465/rmga/2025/2025_04_02/16466_2025_04_02_41162/
    """
    def __init__(self):
        # 存入数据库时不变的字段
        self.init_db_fields()
        ...
        self.start_day = 6
        self.end_day = 8
        self.url_prefix = "https://epaper.cpd.com.cn/szb/wwwcpd_9/dzb_16465/rmga/"
    ...
    def init_db_fields(self):
        ...
        self.chinese_domain = "人民公安报"
        self.attachment_url = None  # 附件链接
        self.source = "人民公安报"  #  文章来源（Part）
    ...
    def init_params(self):
        ...
        self.headers = {
            ...
            # 'if-modified-since': 'Wed, 31 Aug 2022 00:10:24 GMT',
            # 'if-none-match': '"630ea6f0-1a96e"',
            'priority': 'u=0, i',
    ...
    def generate_dates(self, start_year, start_month, start_day, end_year, end_month, end_day):
        ...
        :return: 日期列表
        """
        # 接口链接：https://epaper.cpd.com.cn/szb/wwwcpd_9/dzb_16465/gabsy/suoyin/

        # 访问接口得到所有日期
        date_url = 'https://epaper.cpd.com.cn/szb/wwwcpd_9/dzb_16465/gabsy/suoyin/'
    ...
    def get_details(self, date):
        ...
        # 单日异常
        # url = "https://epaper.cpd.com.cn/szb/wwwcpd_9/dzb_16465/rmga/2020/2020_04_03/16466_2020_04_03_20915/"

        response = self.get_with_retry(url)
        if response is None:
            logger.error(f"Response is None for url: {url}.The page may be empty or pictures.")
            return []
        ...
        pattern = re.compile(r'document\.write\(\'<li><a href="(http://epaper.cpd.com.cn/szb/wwwcpd_9/dzb_16465/rmga/.*?)">', re.DOTALL)
...
if __name__ == '__main__':
    rmga = RMGA()
    dates = rmga.generate_dates(rmga.start_year, rmga.start_month, rmga.start_day, rmga.end_year, rmga.end_month, rmga.end_day)
    for date in dates:
        details = rmga.get_details(date)
    # 多线程处理
        logger.debug(f"成功生成{date}的所有篇章的详细链接，共计{len(details)}篇")
        with ThreadPoolExecutor(max_workers=rmga.max_workers) as executor:
            results = executor.map(rmga.get_data, details)
            try:
