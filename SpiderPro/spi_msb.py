...
class MeiShuBao:
    """
    美术报爬虫
    示例网站：https://msb.zjol.com.cn/html/2025-03/01/node_199.htm
    """
    def __init__(self):
        # 存入数据库时不变的字段
        self.init_db_fields()
        ...
        self.start_day = 1
        self.end_day = 8
        self.url_prefix = "https://msb.zjol.com.cn/html/"
        self.url_suffix_default = "node_199.htm"
    ...
    def init_db_fields(self):
        """存入数据库时不变的字段"""
        self.web_domain = "msb.zjol.com.cn"
        self.chinese_domain = "美术报"
        self.level_column_2 = None  # 文章二级栏目
        self.attachment_url = None  # 附件链接
        self.source = "美术报"  #  文章来源（Part）
    ...
    def get_urls_for_every_branch(self, url, retry=3, timeout=2):
        ...
        :param timeout: 超时时间
        :return: 每日各版面url列表
        版面链接形如：https://msb.zjol.com.cn/html/2025-03/01/node_205.htm
        """
        for attempt in range(retry):
            try:
                response = self.session.get(url, timeout=timeout)
    ...
    def get_detailed_urls(self, url, retry=3, timeout=2):
        ...
        详细链接形如：https://msb.zjol.com.cn/html/2025-03/01/content_4315520.htm?div=-1
        """
        for attempt in range(retry):
            try:
                response = self.session.get(url, timeout=timeout)
    ...
    def get_data(self, url):
        ...
        :param url: 文章的url
        :return: 文章的数据
        示例url：https://msb.zjol.com.cn/html/2022-01/01/content_3984609.htm?div=-1
        """
        # 获取网页内容
        ...
        # 提取一级栏目和版次
        level_column_1_pattern = re.compile(r'(\d+)版：([^<]+)</span>')
        ...
        article_url = url
        # 提取文章标题<h1 class="main-article-title">(.*?)</h1>
        title_match = re.findall(r'<h1 class="main-article-title">(.*?)</h1>', response, re.DOTALL)
        ...
        # 提取图片说明内容
        pic_text_pattern = re.compile(r'<div class="main_ar_pic_text">.*?<IMG.*?<TD>(.*?)</TD>', re.DOTALL)
        pic_text_matches = pic_text_pattern.findall(response)
        pic_text_content = "\n".join([re.sub(r'<[^>]+>', '', match).strip() for match in pic_text_matches])

        # 合并文章内容和图片说明内容
        if article_content:
            article_content = re.sub(r'<[^>]+>', '', article_content)
            article_content = article_content.replace('&nbsp;', '')
        article_content += pic_text_content
        # 作者
...
if __name__ == '__main__':
    msb = MeiShuBao()
    every_day_urls = msb.generate_urls(
        msb.start_year, msb.start_month, msb.start_day, 
        msb.end_year, msb.end_month, msb.end_day
        )
    for every_day_url in every_day_urls:
        if msb.check_url(every_day_url):
            detail_list = []  # 存储每日所有篇章的详细链接
            branches = msb.get_urls_for_every_branch(every_day_url)
            for branch in branches:
                details = msb.get_detailed_urls(branch)
                detail_list.extend(details)
            # 多线程处理
            logger.debug(f"成功生成{every_day_url}的所有篇章的详细链接，共计{len(detail_list)}篇")
            with ThreadPoolExecutor(max_workers=msb.max_workers) as executor:
                results = executor.map(msb.get_data, detail_list)
                try:
